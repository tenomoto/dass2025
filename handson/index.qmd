...
- ステップ $n$ での $\lambda_Z$ は $Z$ に関するモデルの微分と<br>ステップ $n+1$ での $\lambda_Z$との積
...
$$
\begin{aligned}
L&(\mathbf{x},\,z_1, \dots, z_N,\,\lambda_1,\dots,\lambda_N) = J(\mathbf{x},\,z_{1},\dots,z_N)\\&-\lambda_1(z_1 - f_1(\mathbf{x})) - \sum_{n=2}^{N} \lambda_n(z_n - f_n(\mathbf{x}, z_{n-1}))
\end{aligned}
$$
...
- $\partial L/\partial\mathbf{z}$: 随伴モデル
$$\lambda_N = \frac{\partial J}{\partial Z_N},\,\lambda_n = \frac{\partial J}{\partial z_n} + \frac{\partial f_{n+1}}{\partial z_n}\lambda_{n+1}$$
- $\partial L/\partial\mathbf{x}$: $J$の制御変数についての勾配 

$$
\frac{\partial J}{\partial x_k}
= \sum_{i=1}^N \frac{\partial f_i}{\partial x_k}\lambda_i,\,k = 1,\dots, m
$$
